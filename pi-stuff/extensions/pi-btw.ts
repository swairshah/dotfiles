...
class BtwOverlayComponent extends Container implements Focusable {
  ...
  refresh(): void {
    ...
    this.hintsText.setText("Enter submit · Alt+/ minimize/restore · Escape dismiss · PgUp/PgDn scroll");
    this.tui.requestRender();
  }
}
...
export default function (pi: ExtensionAPI) {
  ...
  function buildMinimizedWidgetLines(): string[] {
    const entries = transcriptState.entries;
    const exchanges = getCompletedExchangeCount(entries);
    const activity = hasStreamingTranscriptEntry(entries) ? "streaming" : "idle";
    const modeLabel = pendingMode === "tangent" ? "tangent" : "contextual";
    const statusSuffix = overlayStatus ? ` · ${overlayStatus}` : "";

    return [
      `BTW minimized · ${exchanges} exchange${exchanges === 1 ? "" : "s"} · ${modeLabel} · ${activity}${statusSuffix}`,
      "Press Alt+/ or Ctrl+Alt+W to restore",
    ];
  }
  ...
  function syncUi(ctx?: ExtensionContext | ExtensionCommandContext): void {
    ...
    const minimized = !!handle && (overlayRuntime?.minimized ?? handle.isHidden());
    if (minimized) {
      activeCtx.ui.setWidget("btw", buildMinimizedWidgetLines(), { placement: "belowEditor" });
    } else {
  ...
  function setOverlayMinimized(minimized: boolean, ctx?: ExtensionContext | ExtensionCommandContext): void {
    const handle = overlayRuntime?.handle;
    if (!handle || !overlayRuntime) {
      return;
    }

    overlayRuntime.minimized = minimized;
    if (minimized) {
      handle.unfocus();
  ...
  function toggleOverlayMinimized(ctx?: ExtensionContext | ExtensionCommandContext): void {
    const handle = overlayRuntime?.handle;
    if (!handle) {
      return;
    }

    const hidden = overlayRuntime?.minimized ?? handle.isHidden();
    setOverlayMinimized(!hidden, ctx);
  }
  ...
  function focusOverlay(ctx?: ExtensionContext | ExtensionCommandContext): void {
    ...
    overlayRuntime.minimized = false;
    handle.setHidden(false);
    handle.focus();
    syncUi(ctx);
  }
  ...
  async function ensureOverlay(ctx: ExtensionCommandContext | ExtensionContext): Promise<void> {
    ...
    const runtime: OverlayRuntime = { minimized: false };
    const closeRuntime = () => {
      if (runtime.closed) {
        return;
      }
  ...
  for (const shortcut of BTW_FOCUS_SHORTCUTS) {
    pi.registerShortcut(shortcut, {
      description: "Minimize or restore the BTW overlay without closing the thread.",
      handler: async (ctx) => {
        toggleOverlayMinimized(ctx);
      },
    });
  }
